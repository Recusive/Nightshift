...
class TestReadState:
    ...
    def test_corrupt_category_counts_values_dropped_silently(self, tmp_path):
        ...
        would previously raise ValueError/TypeError. Non-finite floats are also
        skipped, so corrupt values are silently dropped, consistent with how
        ...
        assert "Docs" not in state["category_counts"]
        assert "Polish" not in state["category_counts"]
...
class TestRunNightshiftPaths:
    ...
    def test_rejected_test_mode_run_keeps_target_repo_clean(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.infra.worktree.tempfile.gettempdir", return_value=str(runtime_root)),
...
class TestReadRepoInstructionsTruncation:
    ...
    def test_file_exceeding_per_file_limit_truncated(self, tmp_path: Path) -> None:
        from nightshift.core.constants import MAX_INSTRUCTION_FILE_BYTES
    ...
    def test_total_content_bytes_within_budget(self, tmp_path: Path) -> None:
        from nightshift.core.constants import MAX_INSTRUCTION_FILE_BYTES, MAX_INSTRUCTION_TOTAL_BYTES
...
class TestDiffLineScore:
    def test_security_pattern_scores_high(self) -> None:
        from nightshift.owl.scoring import _diff_line_score
...
class TestHasTestFiles:
    def test_python_test_file(self) -> None:
        from nightshift.owl.scoring import _has_test_files
...
class TestReadRepoMetrics:
    ...
    def test_reads_state_file(self, tmp_path):
        from nightshift.infra.multi import _read_repo_metrics
...
class TestProfilerConstants:
    def test_language_extensions_has_major_languages(self) -> None:
        from nightshift.core.constants import LANGUAGE_EXTENSIONS
    ...
    def test_framework_markers_has_common_frameworks(self) -> None:
        from nightshift.core.constants import FRAMEWORK_MARKERS
...
class TestCountLanguages:
    def test_empty_dir(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _count_languages
...
class TestPrimaryLanguage:
    def test_returns_unknown_for_empty(self) -> None:
        from nightshift.raven.profiler import _primary_language
...
class TestDetectFrameworksByMarker:
    def test_detects_nextjs(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _detect_frameworks_by_marker
...
class TestDetectFrameworksByPackage:
    def test_detects_react_from_package_json(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _detect_frameworks_by_package
...
class TestDetectFrameworks:
    def test_combines_marker_and_package(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _detect_frameworks
...
class TestDetectDependencies:
    def test_detects_requirements_txt_dependencies(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _detect_dependencies
...
class TestFindInstructionFiles:
    def test_finds_claude_md(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _find_instruction_files
...
class TestListTopLevelDirs:
    def test_lists_dirs(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _list_top_level_dirs
...
class TestHasMonorepoMarkers:
    def test_detects_turbo(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _has_monorepo_markers
...
class TestCountTotalFiles:
    def test_counts_files(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _count_total_files
...
class TestDetectConventions:
    def test_detects_python_naming_and_absolute_imports(self, tmp_path: Path) -> None:
        from nightshift.raven.profiler import _detect_conventions
...
class TestRunPlanAgent:
    ...
    def test_agent_not_installed_raises(self, tmp_path: Path) -> None:
        with (
            patch("nightshift.raven.planner.command_exists", return_value=False),
    ...
    def test_agent_nonzero_exit_raises(self, tmp_path: Path) -> None:
        with (
            patch("nightshift.raven.planner.command_exists", return_value=True),
            patch("nightshift.raven.planner.run_command", return_value=(1, "error output")),
    ...
    def test_agent_unparseable_output_raises(self, tmp_path: Path) -> None:
        with (
            patch("nightshift.raven.planner.command_exists", return_value=True),
            patch("nightshift.raven.planner.run_command", return_value=(0, "not valid json")),
    ...
    def test_agent_success_returns_plan(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.planner.command_exists", return_value=True),
            patch("nightshift.raven.planner.run_command", return_value=(0, raw_output)),
        ):
    ...
    def test_agent_invoked_with_correct_cwd(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.planner.command_exists", return_value=True),
            patch("nightshift.raven.planner.run_command", return_value=(0, raw_output)) as mock_run,
        ):
...
class TestDecomposePlan:
    ...
    def test_work_order_has_schema_path(self) -> None:
        ...
        result = nightshift.decompose_plan(plan, profile)
        order = result["waves"][0][0]
        assert order["schema_path"] == "nightshift/schemas/task.schema.json"
...
class TestFormatWorkOrders:
    ...
    def test_includes_schema_path(self) -> None:
        ...
        result = nightshift.decompose_plan(plan, profile)
        output = nightshift.format_work_orders(result)
        assert "nightshift/schemas/task.schema.json" in output
...
def _make_work_order(**overrides: object) -> nightshift.WorkOrder:
    ...
    defaults: dict[str, object] = {
        ...
        "estimated_files": 3,
        "depends_on": [],
        "schema_path": "nightshift/schemas/task.schema.json",
    }
    defaults.update(overrides)
...
class TestBuildSubagentCommand:
    def test_codex_command(self) -> None:
        from nightshift.raven.subagent import _build_subagent_command
        ...
        assert "--json" in cmd
        assert "--output-schema" in cmd
        assert str(Path("/tmp/repo/nightshift/schemas/task.schema.json")) in cmd
...
class TestValidateTaskCompletion:
    def test_valid_done(self) -> None:
        from nightshift.raven.subagent import _validate_task_completion
...
class TestParseTaskCompletion:
    def test_parses_valid_done(self) -> None:
        from nightshift.raven.subagent import _parse_task_completion
...
class TestMakeErrorCompletion:
    def test_creates_blocked_completion(self) -> None:
        from nightshift.raven.subagent import _make_error_completion
...
class TestSpawnTask:
    def test_returns_parsed_completion_on_success(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(0, done_json)):
    ...
    def test_returns_none_on_unparseable_output(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(0, "garbage output")):
            result = nightshift.spawn_task(
                order,
                agent="claude",
                repo_dir=tmp_path,
    ...
    def test_returns_blocked_on_agent_blocked(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(0, blocked_json)):
            result = nightshift.spawn_task(
                order,
                agent="claude",
                repo_dir=tmp_path,
    ...
    def test_nonzero_exit_still_parses(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(1, done_json)):
            result = nightshift.spawn_task(
                order,
                agent="claude",
                repo_dir=tmp_path,
    ...
    def test_creates_log_dir(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(0, _make_done_json(1))):
    ...
    def test_uses_custom_timeout(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(0, _make_done_json(1))) as mock_run:
            nightshift.spawn_task(
                order,
                agent="claude",
                repo_dir=tmp_path,
    ...
    def test_codex_reads_message_file(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.run_command", return_value=(0, "garbage")):
            result = nightshift.spawn_task(
                order,
                agent="codex",
                repo_dir=tmp_path,
...
class TestSpawnWave:
    def test_all_done(self, tmp_path: Path) -> None:
        ...
        with patch(
            "nightshift.raven.subagent.run_command",
            side_effect=lambda cmd, **kw: (0, _make_done_json(kw.get("_tid", 1))),
        ):
            ...
            with patch("nightshift.raven.subagent.spawn_task", side_effect=mock_spawn):
    ...
    def test_exhausts_retries(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.subagent.spawn_task", return_value=None):
            result = nightshift.spawn_wave(
                orders,
                agent="claude",
                repo_dir=tmp_path,
...
class TestStageFiles:
    def test_stages_existing_files(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.integrator.git") as mock_git:
...
class TestRunTestSuite:
    def test_returns_zero_on_success(self, tmp_path: Path) -> None:
        with patch("nightshift.raven.integrator.run_test_command", return_value=(0, "ok")):
    ...
    def test_returns_nonzero_on_failure(self, tmp_path: Path) -> None:
        with patch("nightshift.raven.integrator.run_test_command", return_value=(1, "FAIL error")):
    ...
    def test_timeout_returns_failure(self, tmp_path: Path) -> None:
        with patch(
            "nightshift.raven.integrator.run_test_command", return_value=(1, "Command timed out after 10 seconds")
        ):
...
class TestIntegrateWave:
    def test_passed_when_tests_succeed(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.integrator.git"),
            patch("nightshift.raven.integrator.run_test_command", return_value=(0, "3 passed")),
        ):
    ...
    def test_no_test_runner(self, tmp_path: Path) -> None:
        ...
        with patch("nightshift.raven.integrator.git"):
            result = nightshift.integrate_wave(
                wr,
                repo_dir=tmp_path,
                test_command=None,
    ...
    def test_failed_after_exhausting_fix_attempts(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.integrator.git"),
            patch("nightshift.raven.integrator.run_test_command", return_value=(1, "FAIL src/broken.py broken.py")),
            patch("nightshift.raven.integrator.spawn_task", return_value=_make_task_completion()),
        ):
            result = nightshift.integrate_wave(
                wr,
                repo_dir=tmp_path,
    ...
    def test_fix_agent_succeeds_on_second_attempt(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.integrator.git"),
            patch("nightshift.raven.integrator.run_test_command", side_effect=mock_test_run),
            patch("nightshift.raven.integrator.spawn_task", return_value=_make_task_completion()),
        ):
            result = nightshift.integrate_wave(
                wr,
    ...
    def test_stops_fixing_when_diagnosis_fails(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.integrator.git"),
            patch("nightshift.raven.integrator.run_test_command", return_value=(1, "FAIL unrelated_test")),
        ):
            result = nightshift.integrate_wave(
                wr,
                repo_dir=tmp_path,
    ...
    def test_empty_wave_still_integrates(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.integrator.git"),
            patch("nightshift.raven.integrator.run_test_command", return_value=(0, "ok")),
        ):
            result = nightshift.integrate_wave(
                wr,
                repo_dir=tmp_path,
    ...
    def test_uses_custom_max_fix_attempts(self, tmp_path: Path) -> None:
        ...
        with (
            patch("nightshift.raven.integrator.git"),
            patch("nightshift.raven.integrator.run_test_command", return_value=(1, "FAIL src/f.py f.py")),
            patch("nightshift.raven.integrator.spawn_task", side_effect=counting_spawn),
        ):
...
class TestDetectSmokeTest:
    def test_finds_scripts_smoke_test_sh(self, tmp_path: Path) -> None:
        scripts = tmp_path / "nightshift" / "scripts"
        scripts.mkdir(parents=True)
        (scripts / "smoke-test.sh").write_text("#!/bin/bash\necho ok\n")
        assert nightshift.detect_smoke_test(tmp_path) == "bash nightshift/scripts/smoke-test.sh"
...
class TestRunE2ETests:
    def test_passes_when_tests_pass(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr(
            "nightshift.raven.e2e.run_test_command",
    ...
    def test_runs_smoke_test(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("nightshift.raven.e2e.run_test_command", fake_run)
        result = nightshift.run_e2e_tests(repo_dir=tmp_path, test_command="pytest")
        assert result["status"] == "passed"
        assert result["smoke_test_command"] == "bash nightshift/scripts/smoke-test.sh"
        assert result["smoke_test_exit_code"] == 0
        assert len(calls) == 2
        assert "pytest" in calls
        assert "bash nightshift/scripts/smoke-test.sh" in calls
...
class TestE2EStateRoundTrip:
    def test_e2e_result_persists(self, tmp_path: Path) -> None:
        ...
        state["e2e_result"] = nightshift.E2EResult(
            ...
            test_exit_code=0,
            test_output="all ok",
            smoke_test_command="bash nightshift/scripts/smoke-test.sh",
        ...
        assert loaded["e2e_result"]["smoke_test_command"] == "bash nightshift/scripts/smoke-test.sh"
...
class TestFormatFeatureStatusE2E:
    def test_displays_e2e_section(self) -> None:
        ...
        assert "`bash nightshift/scripts/smoke-test.sh`" in output
...
class TestInjectHints:
    ...
    def test_preserves_original_fields(self) -> None:
        ...
        assert result[0]["schema_path"] == "nightshift/schemas/task.schema.json"
