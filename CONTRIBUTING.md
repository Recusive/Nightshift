...
3. The CI pipeline (`make check`) must pass. It runs ruff, mypy, and pytest against both `nightshift/` and `tests/`.
...
**Do not include**: task IDs from `docs/tasks/` (those are internal to the resident daemon's queue), references to handoff files, or version targets. The resident agent manages those.
...
|------|---------|---------------|
| Lint | `ruff check nightshift/ tests/` | Rules: E, W, F, I, UP, B, SIM, RUF, BLE, S, T20, PT, C4 |
| Format | `ruff format --check nightshift/ tests/` | Consistent formatting |
| Types | `mypy nightshift/` | Strict mode -- full annotations, no `cast()`, no `# type: ignore` |
| Tests | `pytest tests/ -v` | All 663+ tests pass, Python 3.9 and 3.12 |
...
- **Dependency flow**: `types -> constants -> errors -> shell -> config/state -> worktree -> cycle -> scoring -> costs -> cleanup/compact -> multi -> profiler -> planner -> decomposer -> subagent -> integrator -> feature -> cli`. New modules slot into this chain.
- **One concern per module.** Do not add unrelated logic to existing files.
- **No hardcoded data in logic files.** Regex patterns, thresholds, score maps go in `constants.py`.
- **TypedDicts in `types.py`** for all data structures. No raw dicts.
...
- Add to `nightshift/__init__.py` re-exports
- Add to `scripts/install.sh` PACKAGE_FILES
- Add to the structure tree in `CLAUDE.md`

If you add a new config key:
- Add to `DEFAULT_CONFIG` in `constants.py`
- Add to `NightshiftConfig` in `types.py`
- Add to `.nightshift.json.example`
...
- `docs/handoffs/` -- the daemon's session memory
- `docs/tasks/` -- the daemon's work queue
- `docs/vision-tracker/TRACKER.md` -- the daemon updates this after each session
- `docs/prompt/evolve.md` -- the self-improving prompt
- `docs/sessions/` -- session logs
- `docs/healer/` -- system health observations
...
- `nightshift/*.py` -- the Python package (follow the dependency flow)
- `tests/test_nightshift.py` -- tests (always add, never remove without justification)
- `scripts/*.sh` -- shell scripts (keep them thin wrappers)
- `CLAUDE.md` -- if you changed project structure (the daemon will verify)
- `docs/changelog/` -- add entries for your changes under the correct version
...
3. It checks structure, registration, types, tests, safety, and docs (see `.claude/agents/code-reviewer.md`).
...
1. **Do not expect a task ID.** External contributions are not in `docs/tasks/`. Process the PR on its own merits.
2. **Do not expect a handoff.** The contributing agent has no access to the handoff system.
3. **Review using `.claude/agents/code-reviewer.md` rules.** The same quality gates apply.
