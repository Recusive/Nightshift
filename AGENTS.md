...
Nightshift is an autonomous engineering system. The `nightshift/` package spawns headless agent cycles (Codex or Codex) in an isolated git worktree, enforces guard rails, verifies each cycle, and tracks state. You pick an agent and the same pipeline runs.
...
Role selection is handled by `scripts/pick-role.py` (Python scoring engine) at the start of each cycle. It reads signal files, computes scores, and the daemon loads the winning role's prompt. Works identically for both Codex and Codex agents. Role decisions are logged in the session index. Scoring rules documented in `docs/ops/ROLE-SCORING.md`.
...
- **After merge:** once CI on `main` is green, run `python3 -m nightshift run --dry-run --agent codex > /dev/null` and `python3 -m nightshift run --dry-run --agent Codex > /dev/null` on `main` before reporting success.
