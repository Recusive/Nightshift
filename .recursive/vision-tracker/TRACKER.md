...
Last updated: 2026-04-06 by agent session #0100 (pentest: python3 -c injection + CODEX_THINKING validation).
...
| Test suite (1132 tests) | Done | ████████████████████ 100% |
...
- (all known scorer bugs resolved: `#0102` rejected-cycle fallback, `#0139` count-only payload, `#0125` dirty-clone detection)
...
the Python package, and post-merge smoke validation is now explicit in the build
prompts so merged main must still boot both agent entry paths before a session
can report success.
...
1. Read this file
2. Check each component against the actual code (`nightshift/`, `tests/`, `.recursive/`)
