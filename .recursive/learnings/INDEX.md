...
- [Fresh eval evidence drives scheduling gates](2026-04-05-fresh-eval-evidence-drives-gates.md) — When a task changes queue policy based on eval health, rerun Step 0 and cite the latest scored report before editing the prompt
- [Reassess stale eval tasks with a fresh rerun](2026-04-05-reassess-stale-eval-tasks.md) — If an eval-created task tracks tool behavior, rerun the default path first; the right fix may be closing stale queue state instead of adding code
...
- [Codex dual-format stream-json extractors](2026-04-06-codex-dual-format-extractors.md) — Any stream-json parser checking only `type:result` silently fails for Codex; always handle `item.completed/agent_message` as fallback
...
- [.next-id stale after multi-task OVERSEE](2026-04-06-next-id-stale-after-multi-task-session.md) — OVERSEE creating N tasks may leave .next-id at first task number; fix by comparing against highest existing task file at session start
- [Broad fallback regex is an injection vector](2026-04-06-eval-fallback-regex-injection.md) — Loose fallback like `Total.*?(\d+)/100` accepts fabricated single-line files; add a content validity check (date + N structural rows) and remove the fallback instead
...
- [stdout/stderr separation for role selection](2026-04-07-stdout-stderr-separation-role-selection.md) — Never use 2>&1 when capturing structured Python stdout; use mktemp for stderr so unexpected output cannot corrupt the captured value
- [Comprehensive python3 -c audit: fix all or fix none](2026-04-06-comprehensive-python3-c-audit.md) — When fixing one unsafe python3 -c interpolation, grep the whole file for all calls and fix every unsafe instance in the same PR
...
- [Auth failure and circuit breaker](2026-04-06-auth-failure-circuit-breaker.md) — Claude "Not logged in" exits must bypass the consecutive-failure counter; check is_auth_failure before incrementing
