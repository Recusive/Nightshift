# Plan: Recursive Framework + Process Verification Pipeline
...
**Architecture shift:** The meta-layer (daemon, operators, task queue, handoffs, learnings, etc.) is being extracted into `Recursive/` — a portable Autonomous Agentic Layer Framework that can plug into any codebase. Role prompts are now **operators** (Claude Code skill format with SKILL.md + references/) inside `Recursive/operators/`.

**Goal:** Build the Recursive framework structure, create operators from role prompts, add a Process Verification Pipeline that counteracts RLHF biases, and make the agent's reasoning visible, verifiable, and accountable.

## Recursive Structure (completed)

```
Recursive/
├── operators/           # 6 autonomous behaviors (Claude Code skill format)
│   ├── build/           # Ship features, tests, docs, releases
│   ├── review/          # Harden code quality file by file
│   ├── oversee/         # Manage the task queue
│   ├── strategize/      # Big picture analysis and recommendations
│   ├── achieve/         # Measure and improve system autonomy
│   └── security-check/  # Red team / adversarial audit
├── engine/              # Orchestration (daemon, role selection, guards)
├── agents/              # Sub-agent definitions (code review, safety, etc.)
├── prompts/             # Core system rules (autonomous mode, verification)
├── scripts/             # Generic utilities (task validation, rollback, etc.)
└── templates/           # Scaffolds for target project (handoff, task, eval, config)
```
...
### Change B: Recursive Operator Architecture (COMPLETED)

Role prompts extracted into `Recursive/operators/` as Claude Code skills (SKILL.md + references/). Shared boilerplate stripped. Universal rules in `Recursive/prompts/autonomous.md`.

| Operator | Source | Key Content |
|----------|--------|-------------|
| `operators/build/` | evolve.md + BUILD rules from evolve-auto.md | 12-step process, task selection, eval gate, release + 3 reference files |
| `operators/review/` | review.md | Pick file, deep read, fix quality + what-to-look-for reference |
| `operators/oversee/` | overseer.md | Triage queue, close/wontfix/reorder + triage-rules reference |
| `operators/strategize/` | strategist.md | Big picture, cost intelligence, prompt health + prompt-health reference |
| `operators/achieve/` | achieve.md | Autonomy scorecard, eliminate human deps + scorecard reference |
| `operators/security-check/` | pentest.md | Red team scan, CONFIRMED vs THEORETICAL + attack-surface reference |

Also created: `Recursive/engine/` (daemon scripts), `Recursive/agents/` (sub-agent defs), `Recursive/prompts/autonomous.md` (universal rules), `Recursive/templates/` (project scaffolding), `Recursive/scripts/` (generic utilities).

**Migration status:** Files copied to Recursive/. Original files still in place for backward compat. Path switchover is Phase 2b (update daemon.sh `pick_session_role()` + PROMPT_GUARD to use Recursive/ paths).

**PROMPT_GUARD updates (Phase 2b):**
```bash
...
# KEEP existing entries until standalone daemons are removed
```
...
### Phase 2: Recursive operator architecture (Change B) -- COMPLETED
1. Created `Recursive/` directory structure
2. Created 6 operators as Claude Code skills (SKILL.md + references/):
   - `operators/build/` from evolve.md (BUILD rules from evolve-auto.md embedded)
   - `operators/review/` from review.md
   - `operators/oversee/` from overseer.md
   - `operators/strategize/` from strategist.md
   - `operators/achieve/` from achieve.md
   - `operators/security-check/` from pentest.md
3. Created `Recursive/prompts/autonomous.md` (universal rules, stripped of BUILD-specific)
4. Copied engine scripts, agents, generic utilities, created templates
5. TODO (Phase 2b): Update daemon.sh `pick_session_role()` to point to `Recursive/operators/`
6. TODO (Phase 2b): Update lib-agent.sh PROMPT_GUARD for Recursive/ paths
7. Ship as PR, merge
