...
```bash
for f in .recursive/tasks/0*.md; do
  num=$(basename "$f" .md)
...
Read `.recursive/tasks/.next-id`. That file contains a single number — the next available task ID. Use it, then increment it and write the new value back. Always commit `.next-id` alongside the new task file.

```bash
# Read, use, increment
NEXT=$(cat .recursive/tasks/.next-id)
printf "%04d" "$NEXT"   # your task number, zero-padded
echo $(( NEXT + 1 )) > .recursive/tasks/.next-id
```
...
Completed tasks (`status: done`) are automatically moved to `.recursive/tasks/archive/` by the daemon's housekeeping step between sessions. This keeps the active directory small — only pending, in-progress, and blocked tasks remain.

The archive is permanent history. Never delete archived tasks.

## Create the file

`.recursive/tasks/NNNN.md`:
...
4. Check `.recursive/vision-tracker/TRACKER.md` — sections with lower percentages need more tasks
...
See .recursive/vision/01-loop1-hardening.md item #4 for full spec.
```
