...
Last updated: 2026-04-05 by session #0055
...
| `cli.py` | 543 | CLI entry points: run, test, summarize, verify-cycle, module-map. | `run_nightshift`, `summarize`, `verify_cycle_cli`, `plan_feature` | session #0055 |
| `__main__.py` | 5 | Entry point for python3 -m nightshift. | `main` | 2802c51 |
| `__init__.py` | 535 | Nightshift -- autonomous overnight codebase improvement agent. | `AGENT_DEFAULT_MODELS`, `BACKEND_DIR_NAMES`, `BACKEND_EXTENSIONS`, `CATEGORY_ORDER` | PR #89 (7211bd4) |
...
- PR #95: docs: track readme review follow-up
- PR #94: fix: target step 0 evaluations at cloned repos
- PR #93: feat: add task queue summary command
- PR #89: fix: keep hardening cycles inside the worktree
- PR #88: fix: harden healer rotation path handling
