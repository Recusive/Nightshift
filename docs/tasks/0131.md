...
Eval fix tasks (#0097-0102) exist but never get picked because lower-numbered tasks always win in the queue. The latest E2E eval score is still only 70/100, so Nightshift still does not reliably prove itself against real repos. This is the same deprioritization pattern that previously delayed releases.
