...
- **Follow the dependency flow:** `types -> constants -> errors -> shell -> config/state -> worktree -> cycle -> scoring -> costs -> cleanup/compact -> multi -> profiler -> planner -> decomposer -> subagent -> integrator -> summary -> readiness -> e2e -> coordination -> feature -> cli`. New modules slot into this chain. No circular imports. (`multi.py` uses a late import of `run_nightshift` from `cli.py` to avoid circular deps.)
